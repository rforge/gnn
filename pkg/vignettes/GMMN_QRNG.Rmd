...
objname <- paste0("GMMN_dim_",dim.in,"_",dim.hid,"_",dim.out,"_ntrn_",ntrn,
				  ...
				  nu,"_tau_",tau)
if(exists_rda(objname, objnames = objname, package = "gnn")) { # get trained GMMN
    GMMN.mix <- unserialize_model(read_rda(objname, file = objname, package = "gnn"), custom_objects = c(loss = loss))
    ...
    save_rda(serialize_model(GMMN.mix),
...
N01.prior.QRNG <- qnorm(sobol(ngen, d = dim.in, randomize = "digital.shift")) # N(0,1) QRNG
...
objname <- paste0("GMMN_dim_",dim.in,"_",dim.hid,"_",dim.out,"_ntrn_",ntrn,
                 "_nbat_",nbat,"_nepo_",nepo,"_t",nu,"_tau_",taus[2])
if(exists_rda(objname, objnames = objname, package = "gnn")) { # get the saved trained GMMN
    GMMN.t <- unserialize_model(read_rda(objname, file = objname, package = "gnn"), custom_objects = c(loss = loss))
} else { # train the GMMN and save it
    ## Generate a sample from the prior distribution that is fed into the GMMN
    N01.prior <- matrix(rnorm(ntrn * dim.in), nrow = ntrn) # N(0,1) samples
    ## Define the GMMN model
    ...
    print(system.time(GMMN.t %>% fit(x = N01.prior, y = U.t, epochs = nepo,
                                     batch_size = nbat)))
    ## Save the trained GMMN
    save_rda(serialize_model(GMMN.t),
             file = paste0(objname,".rda"), names = objname)
}
...
objname <- paste0("GMMN_dim_",dim.in,"_",dim.hid,"_",dim.out,"_ntrn_",ntrn,
                 ...
                 paste0(ds, collapse = ""),"_tau_",paste0(taus, collapse = "_"))
if(exists_rda(objname, objnames = objname, package = "gnn")) { # get the saved trained GMMN
    GMMN.NG <- unserialize_model(read_rda(objname, file = objname, package = "gnn"), custom_objects = c(loss = loss))
} else { # train the GMMN and save it
    ## Generate a sample from the prior distribution that is fed into the GMMN
    N01.prior <- matrix(rnorm(ntrn * dim.in), nrow = ntrn) # N(0,1) samples
    ## Define the GMMN model
    ...
    print(system.time(GMMN.NG %>% fit(x = N01.prior, y = U.NG, epochs = nepo,
                                      batch_size = nbat)))
    ## Save the trained GMMN
    save_rda(serialize_model(GMMN.NG),
             file = paste0(objname,".rda"), names = objname)
}
```
...
objname <- paste0("GMMN_dim_",dim.in,"_",dim.hid,"_",dim.out,"_ntrn_",ntrn,
                  "_nbat_",nbat,"_nepo_",nepo,"_CvM_statistics_t4_tau_",taus[2],"_G_tau_",taus[2])
if(exists_rda(objname, objnames = objname, package = "gnn")) { # get the saved godness-of-fit statistics
    CvMstat <- read_rda(objname, file = objname, package = "gnn")
} else {
    B <- 100 # number of replications
    CvMstat <- t(rbind( # (4, B) matrix
        replicate(B, CvM(ngen, copula =  t.cop, GMMN = GMMN.t,  dim.in = dim.in)),
        replicate(B, CvM(ngen, copula = NG.cop, GMMN = GMMN.NG, dim.in = dim.in))))
    colnames(CvMstat) <- c("CvM.t.PRNG",  "CvM.t.GMMN.PRNG",
                           "CvM.NG.PRNG", "CvM.NG.GMMN.PRNG")
    save_rda(CvMstat, file = paste0(objname,".rda"), names = objname)
}
```
...
boxplot(list(CvMstat[,"CvM.t.PRNG"], CvMstat[,"CvM.t.GMMN.PRNG"]),
        names = c("Copula PRNG", "GMMN PRNG"), ylab = expression(S[n[gen]]))
...
ES_aggregate_loss <- function(n, copula, GMMN, dim.in, alpha, is.firstkind = FALSE)
{
    ...
    N01.prior.QRNG <- qnorm(sobol(n, d = dim.in, randomize = "digital.shift")) # N(0,1) QRNG samples
    ...
    if (is.firstkind) { # as before but based on a QRNG via the CDM
        S.QRNG <- rowSums(qnorm(cCopula(sobol(n, d = dim.in, randomize = "digital.shift"),
...
objname <- paste0("GMMN_dim_",dim.in,"_",dim.hid,"_",dim.out,"_ntrn_",ntrn,
                  "_nbat_",nbat,"_nepo_",nepo,"_ES99_var_estimates")
if(exists_rda(objname, objnames = objname, package = "gnn")) { # if it exist, get the saved variance estimates
    vars <- read_rda(objname, file = objname, package = "gnn")
    ...
    save_rda(vars, file = paste0(objname,".rda"), names = objname)
}
