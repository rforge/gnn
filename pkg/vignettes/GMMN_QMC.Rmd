...
output:
  ...
  css: style.css
vignette: >
  %\VignetteIndexEntry{Quasi-Monte Carlo for Multivariate Distributions via Generative Neural Networks}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---
<!-- Train on Sharcnet's graham (GPU server) via sbatch start.sh, where the latter contains:
#!/bin/bash
#SBATCH --account=def-mhofert # used to be $USER but needs to be 'def-mhofert' for students to run GPU jobs! might still required to start as 'sbatch --account=def-mhofert start.sh'
...
This vignette accompanies the paper Hofert, Prasad and Zhu (2018, "Quasi-Monte Carlo
for multivariate distributions via generative neural networks"). The aim of this vignette is to
demonstrate in the form of rather minimal examples how select results of this paper
can be obtained. Note that all results of the paper (and more) can be reproduced
with the demo `GMMN_QMC` or the \R package `gnn`. Also note that we omit training of the neural networks (NNs)
here due to time constraints for vignettes (all NNs were pre-trained on a GPU server
with NVIDIA Tesla P100 GPUs).
...
\begin{align*}
  E(\Psi(\mathbf{X}))
\end{align*}
for some measurable $\Psi:\mathbf{R}^d\to\mathbf{R}$ and a random vector $\mathbf{X}=(X_1,\dots,X_d)\sim F_{\mathbf{X}}$ with copula $C$ and margins $F_{X_1},\dots,F_{X_d}$. Our approach utilizes generative moment matching networks (GMMNs) to conceptually construct an approximation $\mathbf{Y}$ to $\mathbf{X}$ and then uses the randomized quasi-Monte Carlo (RQMC) estimator
\begin{align*}
  \frac{1}{n}\sum_{i=1}^n \Psi(\mathbf{Y}_i)
\end{align*} where $\mathbf{Y}_i=f_{\mathbf{\hat{\theta}}}\circ
...
specific $F_{\mathbf{X}}$.  Our goal here is to construct QRNGs for much more general
$F_{\mathbf{X}}$. Training can be done based on any PRNG (which are available for virtually
any high-dimensional model used in practice, in contrast to QRNGs) which makes
this variance reduction approach widely applicable. For more details and
many more examples, we refer to Hofert, Prasad and Zhu (2018) and focus here on
the main steps and how to achieve them in \R.
...
package <- "gnn" # # uses pre-trained NNs from 'gnn' (recommended); retraining if NULL
...
nepo <- 300L # number of epochs (one epoch = one pass through the complete training dataset)
stopifnot(dim.hid >= 1, ntrn >= 1, 1 <= nbat, nbat <= ntrn, nepo >= 1)
```


## 2 A bivariate mixture copula example
...
We now set up the GMMN and train it based on the training dataset
and a sample of equal size from the prior distribution. The idea here is that the
GMMN thus learns the map from a sample from the prior distribution to the
training dataset.
```{r 2.1_training}
## Train the GMMN
...
### 2.2 Visualizing PRNs, GMMN PRNs and GMMN QRNs
...
Moreover, and this
is a rather surprising result, we can use the GMMN to generate quasi-random numbers (QRNs)
from the mixture copula when feeding it with QRNs from the prior (easily
obtained); note that the variance-reduction effect is shown later.
```{r 2.1_sampling}
...
Let us now use these samples to produce plots similar to the top row of Figure 7 of Hofert,
Prasad and Zhu (2018) consisting of a pseudo-random sample from the mixture copula,
a pseudo-random sample from the trained GMMN and a quasi-random sample
from the trained GMMN.
```{r 2.1_plot, fig.align = "center", fig.width = 7.4, fig.show = "hold"}
...
### 2.3 Visually assessing the accuracy of the GMMN samples

We now briefly consider a visual assessment of whether the generated samples from
the trained GMMN are indeed samples from the mixture copula as specified. To this end,
we compute the Rosenblatt transformation of the GMMN PRNs and GMMN QRNs.
```{r, 2.2_Rosenblatt}
...
If the training worked well, the Rosenblatt transformed samples should
produce random numbers from $\mathrm{U}(0,1)^2$. Let us now visually verify this.
```{r 2.2_plot, fig.align = "center", fig.width = 7.4, fig.height = 4, fig.show = "hold"}
...
layout(1) # restore layout
```
The right-hand side plot is similar to the one on the bottom-left
of Figure 4 of Hofert, Prasad and Zhu (2018); see the top-left plot in this
reference for a comparison of the level curves of the empirical copulas based on
the GMMN PRNs and based on the GMMN QRNs with the level curves of the true
copula. The level curves of the empirical copula based on the GMMN QRNs
follow the level curves of the true copula more closely than the ones of
the empirical copula based on the GMMN PRNs.
...
```{r 3.1_generate_training_data}
## Generate training samples from the t_4 copula with a PRNG
set.seed(271) # for reproducibility
U <- rCopula(ntrn, copula = t.cop) # training dataset
```

Next, we set up the GMMN and train it based on the training dataset
and a sample of equal size from the prior distribution.
```{r 3.1_training}
## Train the GMMN
...
## Compute replications of the Cramer-von Mises statistics
...
sampling approaches with a box plot; see the top-left plot in
Figure 8 of Hofert, Prasad and Zhu (2018).
```{r 3.2_plot, fig.align = "center", fig.width = 6, fig.height = 6, fig.show = "hold"}
opar <- par(pty = "s")
...
quasi-Monte Carlo (RQMC) estimator by analyzing its variance reduction
when estimating the risk measure expected shortfall of an aggregate loss
at confidence level $0.99$. To this end, we first define two auxiliary functions
and then compute the standard deviations of four expected shortfall estimators
(one based on a copula PRNG, one based on the GMMN PRNG, the GMMN QRNG and a copula QRNG)
for different sample sizes.
```{r 3.3_auxiliary_functions_and_computation}
...
We now plot the computed standard deviations of the four estimators as functions of
the considered sample sizes $n_{\text{gen}}=\{2^9,2^{9.5},\dots,2^{18}\}$; compare with
the top middle plot in Figure 13 of Hofert, Prasad and Zhu (2018).
```{r 3.3_plot, fig.align = "center", fig.width = 6, fig.height = 6, fig.show = "hold"}
opar <- par(pty = "s") # square plots
