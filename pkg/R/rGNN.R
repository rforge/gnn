...
##' @note rGNN.numeric <- function(n, x, data, ...) would have been another
##'       option but then 'n' is required even if 'data' is a sample, which is
...
##'       more like sample()
rGNN.gnn_GNN <- function(x, size, data = NULL, pobs = FALSE, ...)
{
    stopifnot(inherits(x, "gnn_GNN"), is.logical(pobs))
    if(is.null(data))
        data <- rPrior(size, copula = indepCopula(dim(x)[1]), ...) # independent N(0,1)
    res <- ffGNN(x, data = data)
